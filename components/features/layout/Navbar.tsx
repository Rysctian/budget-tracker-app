...
const tabs = ["dashboard", "income", "expenses", "setup", "settings"]
...
export default function Navbar() {
  ...
  useEffect(() => {
    const currentTabIndex = tabs.indexOf(pathname.slice(1) || 'dashboard')
    if (currentTabIndex !== -1) {
      const activeElement = tabRefs.current[currentTabIndex]
      if (activeElement) {
        const { offsetLeft, offsetWidth } = activeElement
        setActiveStyle({
          left: `${offsetLeft}px`,
